...
def pyarrow_test_helper(establish_connection, n, k):
    ...
    random.seed(n * k)
    df = generate_primitive_df(n, names, schema).sort_values(
        by=["int32col", "int64col", "uint64col", "floatcol"]
    )
...
def test_pyarrow_time(conn_db_readonly: ConnDB) -> None:
    ...
    col2 = pa.array(
        [1000123000000, 2000123000000, 3000123000000], type=pa.duration("us")
    )
    col3 = pa.array(
        [1000123000000000, 2000123000000000, 3000123000000000], type=pa.duration("ns")
    )
    col4 = pa.array(
        [datetime(2012, 1, 20), datetime(2000, 12, 2), datetime(1987, 5, 27)],
        type=pa.timestamp("s"),
    )
    col5 = pa.array(
        [datetime(2012, 1, 20), datetime(2000, 12, 2), datetime(1987, 5, 27)],
...
def test_pyarrow_blob(conn_db_readonly: ConnDB) -> None:
    ...
    col1 = pa.array(
        [generate_blob(random.randint(10, 100)) for i in range(16000)], type=pa.binary()
    )
    col2 = pa.array(
        [generate_blob(random.randint(10, 100)) for i in range(16000)],
...
def generate_string(length):
    ...
    return "".join(
        [
            random.choice("1234567890-=qwertyuiop[]\\asdfghjkl;'zxcvbnm,./")
...
def test_pyarrow_string(conn_db_readonly: ConnDB) -> None:
    ...
    index = pa.array(range(16000), type=pa.int64())
    col1 = pa.array(
        [generate_string(random.randint(10, 100)) for i in range(16000)],
        type=pa.string(),
    )
    col2 = pa.array(
        [generate_string(random.randint(10, 100)) for i in range(16000)],
...
def test_pyarrow_dict(conn_db_readonly: ConnDB) -> None:
    ...
    col1 = pa.array(
        [random.randint(0, 1) for i in range(2000)], type=pa.int32()
    ).dictionary_encode()
    col2 = pa.array(
        [random.randint(-20, 20) / 10 for i in range(2000)], type=pa.float64()
    ).dictionary_encode()
...
def test_pyarrow_list(conn_db_readonly: ConnDB) -> None:
    ...
    col1 = pa.array(
        [
            (
                [
                    generate_primitive("int32[pyarrow]")
                    for x in range(random.randint(1, childlength))
                ]
                if random.randint(0, 5) == 0
                else None
            )
...
def test_pyarrow_list_offset(conn_db_readonly: ConnDB) -> None:
    ...
    values = pa.array(
        [
            generate_primitive("int32[pyarrow]")
            for _ in range(datalength * childlength + 2)
        ]
    )
    offsets = pa.array(
        sorted(
            [
                random.randint(0, datalength * childlength + 1)
                for _ in range(datalength + 1)
            ]
        )
    )
    mask = pa.array([random.choice([True, False]) for _ in range(datalength)])
    col1 = pa.ListArray.from_arrays(
        values=values.slice(2, datalength * childlength), offsets=offsets, mask=mask
    )
    df = pd.DataFrame(
        {
            "index": arrowtopd(index),
...
def test_pyarrow_fixed_list(conn_db_readonly: ConnDB) -> None:
    ...
    primitive_values = pa.array(
        [generate_primitive("int32[pyarrow]") for _ in range(data_len * child_len)]
    )
    primitive_col = pa.FixedSizeListArray.from_arrays(
        primitive_values, list_size=child_len, mask=mask
    )
    ...
    datetime_values = pa.array(
        [
            datetime(
                random.randint(1900, 2023), random.randint(1, 12), random.randint(1, 28)
            )
            for _ in range(data_len * child_len)
        ],
        type=pa.date32(),
    )
    datetime_col = pa.FixedSizeListArray.from_arrays(
        datetime_values, list_size=child_len, mask=mask
    )

    # fixed list of blob
    blob_values = pa.array(
        [generate_blob(random.randint(10, 100)) for _ in range(data_len * child_len)],
        type=pa.binary(),
    )
    blob_col = pa.FixedSizeListArray.from_arrays(
        blob_values, list_size=child_len, mask=mask
    )

    # fixed list of string
    string_values = pa.array(
        [generate_string(random.randint(10, 100)) for _ in range(data_len * child_len)],
        type=pa.string(),
    )
    string_col = pa.FixedSizeListArray.from_arrays(
        string_values, list_size=child_len, mask=mask
    )
    ...
    dict_col = pa.FixedSizeListArray.from_arrays(
        dict_values, list_size=child_len, mask=mask
    )
    ...
    list_col = pa.FixedSizeListArray.from_arrays(
        list_values, list_size=child_len, mask=mask
    )

    # fixed list of fixed list
    fixed_list_col = pa.FixedSizeListArray.from_arrays(
        primitive_col, list_size=1, mask=mask
    )
    ...
    struct_col = pa.FixedSizeListArray.from_arrays(
        struct_values, list_size=child_len, mask=mask
    )
    ...
    map_values = pa.array(
        [
            (
                {
                    str(key): "".join(
                        random.sample(valueSet, random.randint(0, len(valueSet)))
                    )
    ...
    map_col = pa.FixedSizeListArray.from_arrays(
        map_values, list_size=child_len, mask=mask
    )
        ...
        proc = [
            ...
            (
                ...
                if map_col[idx].as_py() is None
                    ...
                    (
                        None
                        if map_col[idx][i].as_py() is None
                        else dict(map_col[idx][i].as_py())
                    )
...
def test_pyarrow_fixed_list_offset(conn_db_readonly: ConnDB) -> None:
    ...
    child_len = 5
    values = pa.array(
        [generate_primitive("int32[pyarrow]") for _ in range(data_len * child_len + 2)]
    )
    mask = pa.array([random.choice([True, False]) for _ in range(data_len)])
    index = pa.array(range(data_len))
    col1 = pa.FixedSizeListArray.from_arrays(
        values.slice(2, data_len * child_len), list_size=child_len
    )
    col1 = col1.slice(1, 49)
    col2 = pa.FixedSizeListArray.from_arrays(
        values.slice(1, data_len * child_len), list_size=child_len, mask=mask
    )
...
def test_pyarrow_struct_offset(conn_db_readonly: ConnDB) -> None:
    ...
    index = pa.array(range(datalength))
    val1 = pa.array(
        [generate_primitive("int32[pyarrow]") for _ in range(datalength + 1)]
    )
    val2 = pa.array(
        [generate_primitive("bool[pyarrow]") for _ in range(datalength + 2)]
    )
    val3 = pa.array(
        [generate_string(random.randint(5, 10)) for _ in range(datalength + 3)]
    )
    mask = pa.array([random.choice([True, False]) for _ in range(datalength)])
    col1 = pa.StructArray.from_arrays(
        [
            val1.slice(1, datalength),
            val2.slice(2, datalength),
            val3.slice(3, datalength),
        ],
        names=["a", "b", "c"],
        mask=mask,
    )
...
def test_pyarrow_union_sparse(conn_db_readonly: ConnDB) -> None:
    ...
    index = pa.array(range(datalength))
    type_codes = pa.array(
        [random.randint(0, 2) for i in range(datalength)], type=pa.int8()
    )
    arr1 = pa.array(
        [generate_primitive("int32[pyarrow]") for i in range(datalength + 1)],
        type=pa.int32(),
    )
    arr2 = pa.array(
        [generate_string(random.randint(1, 10)) for i in range(datalength + 2)]
    )
    arr3 = pa.array(
        [generate_primitive("float32[pyarrow]") for j in range(datalength + 3)]
    )
    col1 = pa.UnionArray.from_sparse(
        type_codes,
        [
            arr1.slice(1, datalength),
            arr2.slice(2, datalength),
            arr3.slice(3, datalength),
        ],
    )
    df = pd.DataFrame({"index": arrowtopd(index), "col1": arrowtopd(col1)})
    result = conn.execute("LOAD FROM df RETURN * ORDER BY index")
...
def test_pyarrow_map_offset(conn_db_readonly: ConnDB) -> None:
    ...
    index = pa.array(range(datalength))
    offsets = sorted(
        [random.randint(0, datalength * maplength + 1) for _ in range(datalength + 1)]
    )
    offsets[25] = None
    offsets = pa.array(offsets, type=pa.int32())
    keys = pa.array(
        [random.randint(0, (1 << 31) - 1) for _ in range(datalength * maplength + 1)]
    )
    values = pa.array(
        [
            generate_primitive("int64[pyarrow]")
            for _ in range(datalength * maplength + 1)
        ]
    )
    col1 = pa.MapArray.from_arrays(
        offsets,
        keys.slice(1, datalength * maplength),
        values.slice(1, datalength * maplength),
    )
...
def test_pyarrow_decimal(conn_db_readwrite: ConnDB) -> None:
    ...
    index = pa.array(range(datalength))
    decimal52 = pa.array(
        map(Decimal, ["1.2", "2", "0.5", "100"]), type=pa.decimal128(7, 2)
    )
    decimal380 = pa.array(
        map(Decimal, ["2938103", "109283091238", "1028391238012", "1283019283123"]),
...
def test_pyarrow_skip_limit(conn_db_readonly: ConnDB) -> None:
    ...
    col1 = pa.array(
        [generate_string(random.randint(1, 100)) for _ in range(datalength)]
    )
    col2 = pa.array(
        [
            [generate_primitive("bool[pyarrow]") for x in range(random.randint(1, 10))]
...
def test_pyarrow_invalid_skip_limit(conn_db_readonly: ConnDB) -> None:
    ...
    with pytest.raises(
        RuntimeError,
        match=re.escape(
            "Binder exception: SKIP Option must be a positive integer literal."
        ),
    ):
    ...
    with pytest.raises(
        RuntimeError,
        match=re.escape(
            "Binder exception: LIMIT Option must be a positive integer literal."
        ),
    ):
