...
def udf_helper(conn, functionName, params, expectedResult):
    ...
    result = conn.execute(
        queryString, {str(i + 1): params[i] for i in range(len(params))}
    )
...
def udf_predicate_test(conn, functionName, params, expectedResult):
    ...
    result = conn.execute(
        f"MATCH (t:tbl) WHERE {functionName}(t.id % 8) RETURN COUNT(*)"
    )
    assert result.get_next()[0] == expectedResult
...
def test_udf(conn_db_readwrite: ConnDB) -> None:
    ...
    udf_helper(
        conn, "datetimeToDate", [datetime(2024, 4, 25, 15, 39, 20)], date(2024, 4, 25)
    )
    udf_helper(
        conn,
        "addToDate",
        [
            datetime(2024, 4, 25, 15, 39, 20),
            datetime(2025, 5, 26) - datetime(2024, 4, 25, 15, 39, 20),
        ],
        datetime(2025, 5, 26),
    )
    udf_helper(
    ...
    udf_helper(
        conn, "UDFDist", [{"x": 1, "y": 10}, {"x": 4, "y": 6}], {"x": 9, "y": 16}
    )
...
def test_udf_remove(conn_db_readwrite: ConnDB) -> None:
    ...
    with pytest.raises(
        RuntimeError, match=r"Catalog exception: function notmyfunction doesn't exist."
    ):
    ...
    with pytest.raises(
        RuntimeError, match=r"Catalog exception: function list_create doesn't exist."
    ):
